...
let datas:string[] = []; // 通过lineBreak分割后的字符数组
...
function activate(context: vscode.ExtensionContext) {
  ...
  // 读取文件并插入注释
  const disposable = vscode.commands.registerCommand("constReadNovel.constReadNovelStart", function () {
    fileData = "";
    datas = [];
    curPage = 0;
    ...
    // const replaceMark = vscode.workspace.getConfiguration().get("constReadNovel.replaceMark");
  ...
  // 下一页
  const nextPage = vscode.commands.registerCommand("constReadNovel.ReadNovel_nextPage", function () {
    currentText = datas[++curPage];
  });

  // 上一页
  const prevPage = vscode.commands.registerCommand("constReadNovel.ReadNovel_prevPage", function () {
    currentText = datas[--curPage];
  });

  // 清空注释（老板键）
  const clear = vscode.commands.registerCommand("constReadNovel.ReadNovel_clear", function () {
    currentText = "CONST: The value of a constant canmott be changed through reassignment";
  });
...
function saveReadProgress() {
  ...
  if (datas[0].indexOf(tipTxt) === -1) {
    ...
    ;
    const savePage = Number(datas[0].split(tipTxt)[0]);

    if (curPage === savePage) {
      return;
    }

    const oldLine = savePage + tipTxt + "\r\n";
    const newLine = curPage + tipTxt + "\r\n";
    const newData = fileData.replace(oldLine, newLine);
...
// 检测合法性
function isLegal() {
  if (datas.length === 0 || curPage < 0 || curPage > datas.length) {
    return false;
  }
